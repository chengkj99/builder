...
const makeRule = (extension, context, extendParsePaths, ...loaderList) => {
  const rule = {
  ...
  let excludeRule = /(node_modules)/
  if (extendParsePaths.length > 0) {
    const extendParsePathsJoined = extendParsePaths.join("|").replace(
      /[-[\]{}()*+?.,\\^$|#\s]/g,
      '\\$&'
    )
    excludeRule = new RegExp(`node_modules\/(?!(${extendParsePathsJoined})\/).*`, 'g')
 }
...
module.exports = (config, key, transform, extendParsePaths, post) => {
  ...
  switch(transform.transformer) {
    ...
    case transforms.stylus: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, extendParsePaths, ...loaders)]
        } }
      })
      break
    }
    ...
    case transforms.babel: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, extendParsePaths, { loader: 'babel', options: transform.config })]
        } }
      })
      break
    }
    ...
    case transforms.jsx: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, extendParsePaths, {
            loader: 'babel',
    ...
    case transforms.flow: {
      ...
      config = update(config, {
        module: { rules: {
          $push: [makeRule(extension, context, extendParsePaths, { loader: transform.transformer, options: transform.config })]
        } }
      })
      break
    }
    ...
    case transforms.file: {
      config = update(config, {
        module: { rules: {
          $push: [
            makeRule(extension, context, extendParsePaths, {
              loader: 'file',
